...
// calibrate_* tools) is the gold standard when an operator will rank one;
// these constraints are what the operator ALREADY said, written down, so the
// rubric can be held to its own evidence on every run without asking anyone
// to rank anything.

import { rankRows, type TriageRow } from './jev.js';
...
// A preference is judged only when both files were scored in this run; the
...
export const evaluatePreferences = (rows: TriageRow[], prefs: Preference[]): EvalOutcome => {
  ...
  const rank = new Map(order.map((r, i) => [r.file, i]));
